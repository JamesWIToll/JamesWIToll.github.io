<script setup>
import {onMounted, ref} from "vue";
import {Shader} from "@/models/Shader.js";
import {Renderer} from "@/models/Renderer.js";
import importer from "@/models/Importer.js";

const props = defineProps({model: String});

const canvas = ref(null);
const renderer = ref(null);
const scene = ref(null);
const focused = ref(false);
let gl = null;

const focus = (obj) => {
  focused.value = !focused.value;
  if (focused.value) {
    obj.target.requestPointerLock();
  } else {
    document.exitPointerLock();
  }
}

const move_amt = 0.1;
const mouse_sensitivity = 0.1;
window.addEventListener("keypress", (e) => {
  if (document.pointerLockElement !== null) {
    if (e.key === "D" || e.key === "d") {
      scene.value._currCamera.move("left", move_amt);
    }
    if (e.key === "A" || e.key === "a") {
      scene.value._currCamera.move("right", move_amt);
    }
    if (e.key === "W" || e.key === "w") {
      scene.value._currCamera.move("forward", move_amt);
    }
    if (e.key === "S" || e.key === "s") {
      scene.value._currCamera.move("backward", move_amt);
    }

    if (e.key === "E" || e.key === "e") {
      scene.value._currCamera.move("up", move_amt);
    }
    if (e.key === "Q" || e.key === "q") {
      scene.value._currCamera.move("down", move_amt);
    }
  }
});
window.addEventListener("mousemove", (e) => {
  if(document.pointerLockElement !== null) {
    scene.value._currCamera.look("horizontal", mouse_sensitivity*e.movementX);
    scene.value._currCamera.look("vertical", -mouse_sensitivity*e.movementY);
  }
});

onMounted(async () => {
  gl = canvas.value.getContext('webgl2');
  if(!gl) {
    alert("WebGL2 not supported!");
    return;
  }

  let vertSrc = document.getElementById("vertexShader").innerText;
  let fragSrc = document.getElementById("fragmentShader").innerText;

  let vStartPos = vertSrc.indexOf("#version");
  vertSrc = vertSrc.substring(vStartPos, vertSrc.length);

  let fStartPos = fragSrc.indexOf("#version");
  fragSrc = fragSrc.substring(fStartPos, fragSrc.length);

  let prog = new Shader(gl, vertSrc, fragSrc);
  renderer.value = new Renderer(gl);
  renderer.value.setShaderProgram(prog);


  scene.value = await importer.importGLTF2(props.model, gl);

  renderer.value.setCurrentScene(scene.value);
  requestAnimationFrame(renderer.value.render);

});
</script>

<style scoped>

canvas {
  min-height: 300px;
  border-radius: 10px;
  margin: 10px;
}

</style>

<template style="align-content: center;">

  <canvas ref="canvas" @click="focus"></canvas>

</template>
